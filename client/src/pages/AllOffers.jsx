...
const AllOffers= () => {
  ...
  const[selectedFilters,setSelectedFilters]=useState({
    roomType:[],
    priceRanges:[], // Note: plural 'priceRanges'
  });
  ...
  //Filter and sort rooms based on selected  sort option
  const filteredOffers=useMemo(
    ()=>{
      return offers.filter(room=>matchesRoomType(room) && matchesPriceRange(room) && filterDestination(room)).sort(sortRooms);
    },[offers,selectedFilters,selectedSort,searchParams]
  )
  ...
  return (
    ...
    <div>
      ...
      <h1 className='font-playfair text-4xl md:text-[40px]'>Hotel Offers</h1>
      ...
      {filteredOffers.map((offer)=>(
        <div key={offer._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
          <img onClick={()=>{navigate(`/offers/${offer._id}`); scrollTo(0,0)}}
              src={offer.room.images[0]} alt='hotel-img' title='View Room Detail'
            className='max-h-65 md:w-1/2  rounded-xl shadow-lg object-cover cursor-pointer' />
          <div className='md:w-1/2 flex flex-col gap-2'>
              <p className='text-gray-500'>{offer.hotel.city}</p>
              <p onClick={()=>{navigate(`/offers/${offer._id}`); scrollTo(0,0)}}
          ...
          <p className='ml-2'> {averageRating(testimonials,offer).count}+ reviews</p>
              ...
              <div>
                ...
                <p className='text-xl font-medium text-gray-700'>${offer.currentPricePerNight} /night</p>
              </div>
          </div>
        </div>
      ))}
          ...
          </p>
          {
            priceRanges.map((range,index)=>(
              <CheckBox key={index} label={`${currency} ${range}`} selected={
                selectedFilters.priceRanges.includes(range)
              } onChange={(checked)=>handleFilterChange(checked,range,'priceRanges')} /> // Fixed: 'priceRanges'
            ))
          }
        </div>
        ...
        <div className='px-5 pt-5 pb-7'>
          ...
          </p>
          {
            sortOptions.map((option,index)=>(
              <RadioButton key={index} label={option} selected={
                selectedSort===option
