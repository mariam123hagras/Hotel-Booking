...
try {
  ...
  const {data} =await axios.post('/api/bookings/check-availability',{offer:id,checkInDate,checkOutDate})
  if(data.success){
    if(data.isAvailable){
      setIsAvailable(true)
      toast.success('Offer is available')
    }
    else{
      setIsAvailable(false)
      toast.error('Offer is not available')
    }
  }
  else{
...
try {
  ...
  useEffect(() => {
    ...
    if (offers && Array.isArray(offers)) {
      const foundOffer = offers.find(offer => offer._id === id)
      if (foundOffer) {
        setOffer(foundOffer)
        setMainImage(foundOffer.room.images[0])
      } else {
        console.log('Offer not found with id:', id)
      }
    } else {
      console.log('Offers is not available yet:', offers)
    }
  }, [offers, id])
    ...
    return (
      <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32 flex justify-center items-center'>
        <div className='text-center'>
          <p className='text-lg'>Loading offer details...</p>
          {!offers && <p className='text-sm text-gray-500 mt-2'>Waiting for offers data to load</p>}
        </div>
      </div>
    )
  }
  ...
  return offer && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32' >
      ...
      <div className='flex flex-col md:flex-row items-start md:items-center gap-2'>
        <h1 className='text-3xl md:text-4xl font-playfair' >{offer.hotel.name} <span className='font-inter text-sm'>({offer.room.roomType})</span></h1>
        <p className='text-xs font-inter py-1.5 px-3 text-white bg-orange-500 rounded-full'>{offer.discountValue}% OFF</p>
      </div>
      ...
      <div className='flex flex-col md:flex-row md:justify-between mt-10'>
        <div className='flex flex-col'>
          ...
          <div className='flex flex-wrap items-center mt-3 mb-6 gap-4' >
            {offer.room.amenities.map((item, index) => (
        ...
        {/*Offer Price */}
        <p className='text-2xl font-medium'>${offer.currentPricePerNight} /night</p>

      </div>
      {/* Checking CheckOut Form */}
      <form onSubmit={onSubmitHandler}
       className='flex flex-col md:flex-row  items-start md:items-center
justify-between  bg-white shadow-[0px_0px_20px_rgba(0,0,0,.15)] p-6 rounded-xl  mt-16 max-w-6xl' >
      ...
      outline-none' required />
          ...
          <div className='flex flex-col'>
            ...
            type="number" id='guests' placeholder='1'
              className='max-w-20 rounded border boreder-gray-300 px-3 py-2 mt-1.5
      outline-none' required />
          </div>

        </div>
        <button type='submit' className='bg-blue-600 hover:bg-primary-dull
  ...
  max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer'>
      ...
      <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500'>
        <p>
          ...
          to get the exact price for groups .The Guests will be allocated ground floor according to availability. you get the comfortable two bedroom apartment that has a true city feeling
        </p>
      </div>
      {/* Hosted by */}
      <div className='flex flex-col items-start gap-4' >
        <div className='flex gap-4'>
          <img src={assets.homeIcon} alt="Host" className='
          h-14 w-14 md:h-18  ' />
          <div>
